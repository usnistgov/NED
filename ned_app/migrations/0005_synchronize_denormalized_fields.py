...
def forwards_func(apps, schema_editor):
    ...
    for ref in Reference.objects.all():
        try:
            ...
            if ref.csl_data:
                ...
                if 'issued' not in ref.csl_data or 'date-parts' not in ref.csl_data['issued'] or not ref.csl_data['issued']['date-parts']:
                    raise ValidationError("csl_data must contain a valid 'issued' field with 'date-parts'")

                # Population
                ref.title = ref.csl_data.get('title', '')
                ref.year = ref.csl_data.get('issued', {}).get('date-parts', [[None]])[0][0]

                authors = ref.csl_data.get('author', [])
                if authors:
                    family_names = [author.get('family') for author in authors if author.get('family')]
    ...
    print(f"Synchronization completed. Processed: {processed_count}, Errors: {error_count}")
