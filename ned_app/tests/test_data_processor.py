...
class DataFilesProcessorTest(TestCase):
    ...
    def test_load_data_success(self):
        ...
        with patch('ned_app.serialization.data_files_processor.build_json_data_file_path') as mock_path:
    ...
    @patch('ned_app.serialization.data_files_processor.PROCESS_FRAGILITY_CURVES', False)
    @patch('ned_app.serialization.data_files_processor.PROCESS_EXPERIMENT_FRAGILITY_PAIRS', False)
    @patch('ned_app.serialization.data_files_processor.PROCESS_EXPERIMENTS', False)
    @patch('ned_app.serialization.data_files_processor.PROCESS_FRAGILITY_MODELS', False)
    ...
    def test_import_avail_data_success_with_csl_data(self):
        ...
        with patch('ned_app.serialization.data_files_processor.load_data') as mock_load:
            mock_load.return_value = self.valid_reference_data

            with patch('ned_app.serialization.data_files_processor.ReferenceSerializer') as mock_serializer_class:
    ...
    def test_import_avail_data_missing_csl_data_field(self):
        ...
        with patch('ned_app.serialization.data_files_processor.load_data') as mock_load:
            ...
            self.assertIn("missing required 'csl_data' field", str(context.exception))
            ...
            with patch('ned_app.serialization.data_files_processor.ReferenceSerializer') as mock_serializer_class:
                ...
                with patch('builtins.open', mock_open(read_data='{"type": "array"}')):
                    with patch('jsonschema.validate') as mock_validate:
                        ...
                        mock_serializer_class.assert_called_once_with(data=complex_data[0])
        ...
        for i, case in enumerate(test_cases):
            with self.subTest(case=i):
                with patch('ned_app.serialization.data_files_processor.load_data') as mock_load:
                    ...
                    with patch('ned_app.serialization.data_files_processor.PROCESS_REFERENCES', True):
                        if case["should_fail"]:
                            with self.assertRaises(DataFileDeserializationError) as context:
                                import_avail_data()

                            if case["error_contains"]:
                                self.assertIn(case["error_contains"], str(context.exception))
                        else:
