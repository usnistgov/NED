...
from db import get_component_detail, get_component_experiments, get_component_fragility_models
...
def render() -> None:
    ...
    attr("Id", fmt(row["id"]))
    attr("Component type name", fmt(row["name"]), "Name of the individual type of building component.")
    attr("Major Group", strip_prefix(row["major_group"]))
    attr("Group", strip_prefix(row["group"]))
    attr("Element", strip_prefix(row["element"]))
    attr("NISTIR Sub Element", strip_prefix(row["subelement"]), "NISTIR taxonomy subelement classification.")
    ...
    if df_fm.empty:
        ...
        _FM_HEADERS = ["Model ID", "P-58 Fragility", "Component Detail",
                       "Material", "Size Class", "Component Description", ""]
        ...
        for _, fmrow in df_fm.iterrows():
            ...
            for ci, val in zip(c[:6], [
                fmrow["Model ID"], fmrow["P-58 Fragility"], fmrow["Component Detail"],
                fmrow["Material"], fmrow["Size Class"], desc_short,
            ]):
                ci.markdown(f"<span style='font-size:0.88rem;'>{esc(val)}</span>", unsafe_allow_html=True)
            if c[6].button("View", key=f"fm_{fmrow['fragility_model_id']}"):
                st.session_state["selected_fragility_model_id"] = fmrow["fragility_model_id"]
    ...
    if df_exp.empty:
        ...
        _EXP_HEADERS = ["Specimen", "Test Type", "Location", "EDP Metric", "EDP Value", "DS Class", ""]
        ...
        for _, erow in df_exp.iterrows():
            ...
            for ci, val in zip(c[:6], [
                erow["Specimen"], erow["Test Type"], erow["Location"],
                erow["EDP Metric"], erow["EDP Value"], erow["DS Class"],
            ]):
                ci.markdown(f"<span style='font-size:0.88rem;'>{esc(val)}</span>", unsafe_allow_html=True)
