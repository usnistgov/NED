...
def _lognormal_curves(df_curves: pd.DataFrame, n_points: int = 200) -> pd.DataFrame | None:
...
def render_model_body(
    ...
    if df_curves.empty:
        ...
        if plot_df is not None:
            ...
            fig = go.Figure()
            for ds_label, group in (
                plot_df.sort_values("_rank").groupby("Damage State", sort=False)
            ):
                fig.add_trace(
                    go.Scatter(
                        x=group["EDP"],
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}curves_chart")
        else:
