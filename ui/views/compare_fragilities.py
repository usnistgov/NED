...
def _format_ds_value(attr_label: str, value) -> str:
    if _is_blank(value):
        return '—'
...
def _render_damage_state_table(
    ...
    # string up front rather than relying on per-column formatting.
    display = table.apply(
        lambda row: row.map(lambda v: _format_ds_value(row.name, v)), axis=1
    )
...
def render() -> None:
    ...
    _render_damage_state_table(curves_left, curves_right, label_left, label_right)
