...
def build_citation(csl: dict) -> str:
    ...
    authors = _join_authors([_format_author(a) for a in csl.get("author", []) if isinstance(a, dict)])

    year = ""
    dp = csl.get("issued", {}).get("date-parts") if isinstance(csl.get("issued"), dict) else None
