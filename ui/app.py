...
if os.environ.get("AUTH_ENABLED", "true").lower() != "false" and not auth.check_password():
    st.stop()
...
if "experiment" in st.query_params:
    ...
    st.session_state["selected_fragility_model_id"] = st.query_params["fragility_model"]
    st.session_state["selected_component_id"] = st.query_params.get("component", "")
